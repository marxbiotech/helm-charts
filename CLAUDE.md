...
1. 建立 `charts/<chart-name>/`，包含 `Chart.yaml`、`values.yaml`、`templates/`
2. 以 `charts/sample-app/` 為參考範本
3. `Chart.yaml` 使用 `apiVersion: v2`，必須含 `maintainers`
4. `_helpers.tpl` 命名慣例：`<chart-name>.name`、`<chart-name>.fullname`、`<chart-name>.labels`、`<chart-name>.selectorLabels`、`<chart-name>.serviceAccountName`
